...
export async function getStaticProps() {
    ...
    const getResultsCurrentYearDirectories = await fetch(`https://api.infomaniak.com/2/drive/${process.env.KDRIVE_ROOT}/files/search?with=path&types[]=dir&query="${currentYear}"&directory_id=${process.env.KDRIVE_GEW_RES}`, {
        method: "GET",
        headers: {
    ...
    const getInvitationCurrentYearDirectories = await fetch(`https://api.infomaniak.com/2/drive/${process.env.KDRIVE_ROOT}/files/search?with=path&types[]=dir&query="${currentYear}"&directory_id=${process.env.KDRIVE_GEW_INV}`, {
        method: "GET",
        headers: {
            Authorization: `Bearer ${process.env.KDRIVE}`,
            "Content-Type" : "application/json"
